...
function ValidateControls(idlist) {
    ...
    if (wardlist.length > 0) {
        for (var j = 0; j < wardlist.length; j++) {
            ...
            var txtbox = 'ctl00_Contentbody_' + wardid + month + 'txtNumarator';
        ...
        if (valid) {
            ...
            var response = Views_KPI_SpecialtyLevelDataBulkKPI.SaveBulkData(Numarator, Denominator, YTD, document.getElementById('ctl00_Contentbody_hdnHospitalId').value, document.getElementById('ctl00_Contentbody_hdnSpecialtyId').value, document.getElementById('ctl00_Contentbody_hdnYear').value);
